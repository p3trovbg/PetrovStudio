...
namespace PetrovStudio.Services;
    ...
    public async Task<string> ProcessImageAsync(ImageDto image, CancellationToken ct)
    {
        ...
        switch (image.Stream)
        {
            case { CanSeek: true, Length: > MaxFileSizeInBytes }:
                throw new ArgumentException("File size exceeds the maximum allowed limit of 5MB.", nameof(image.Stream));
        ...
        {
            throw new ArgumentException($"File type '{extension}' is not allowed. Only JPG, JPEG, PNG, and WEBP are supported.");
        }
